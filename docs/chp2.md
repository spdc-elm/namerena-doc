...
省去了初步开箱的麻烦，但某种意义上也是失去了初步开箱的乐趣。（开箱的正式介绍见 2.4，这里你可以理解为将符合条件（一般是三级表情）拿去测 10% 强评的操作）  
...
一个更加诗意的解释是：最开始发明的时候没有测 10% 强评的功能，只能输出全部三表，然后等人回到电脑前之后再用开箱脚本开箱。与当时的文哥直接输出 10% 强评相比，在观测之前没人知道里面到底有多强的号，所以称为“量子”。<br>
量子波动测号器不再基于官测，而是将手测的过程换成用 javascript 脚本实现，比按键精灵和官测快很多。同时也能测 10% 评分并输出高于一定阈值的号。<br>
...
把核测号器的代码进行改写并在显卡上运行，可以获得极为恐怖的测号速度（理论上一块显卡一天可以测几百万亿）。<br>
但显卡测号需要昂贵的高端 GPU 矿机（甚至是集群）和大量电能，并且很容易烧坏显卡，风险巨大。
...
pl 测号器测号的格式为 **前缀（可选）+ 变化字符 + 后缀（可选）+ @ + 队名**。
...
第 1 步：**输入你想要测的测号量**（可以参考下一节“测号单位制”）；
...
- **请确保 `[字符集长度] 的 [可变长度] 次方` 远大于你的测号量（至少一千倍大），避免出现随机重复的情况！！！**
...
- 注意区间为**左闭右开**。
<br><br>
- 注意需要以十进制格式输入。例如，对于 26 位大写字母 A ~ Z 构成的字符集，AAAAA ~ ZZZZZ 的 26 进制区间对应十进制的 0 ~ 11881375，你的输入应该是 `0 11881376`。
...
下面是一些使用加速技术的网站，测评分方法和原版名竞相同（使用 `!test!`）。
...
也有和官方网站相同，可以一直往下测的网站，例如 <https://fast-namerena.pages.dev/>。<br>
*注：这个网站处于动态开发中，较不稳定，可能有评分错误等问题，不建议用于大规模开箱。*
...
工具解压后打开 index.html 便可以测虚评，第一个数值是强评拟合值（称为虚评/xp），第二个数值是强单拟合值（称为虚单/xd）。通过自己修改 main.js 代码可以增设筛选条件和技能提醒等功能。

convert.html 提供了便捷的名字信息转换功能，可以计算得出名字的各项属性和技能熟练度。

值得注意的是，由于虚评和虚单的训练集都是比较强的常规名字，因此如果用虚评器评测离训练集差距较大的名字（例如负榜号）可能会出现离谱的数值。
