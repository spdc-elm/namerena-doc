...
名竞的官方网址为 [deepmess.com/namerena](https://deepmess.com/namerena/)。如果你无法访问上面的网页，可以试试由玩家制作的镜像站 [namerena.pages.dev](https://namerena.pages.dev)。需要注意的是，一些网站上的名竞版本是过时的，你可以通过对比战斗结果的方式来区分新旧版。

如果你看不懂这几段话，或者只是想粗略地了解一下这个游戏，可以阅读一下[入门 Q&A](doc-qa-for-noobs.md)。它虽然是在特殊的社区事件背景下仓促编写的，但仍然有一定参考价值。
...
输入中前面两个号组成一队，后面两个号组成另一队。同一队的名字之间不会互相攻击，只需要击倒所有其他队伍的名字就能胜利（只要有队友活到最后即可，就算自己已经被击倒了，也会成为胜利者）。
...
该现象被称为“组队加成”。当**同一个战队**的多个号在组队战斗时被放在同一队中（可以掺杂其他战队的号），就有概率在其中产生组队加成，对部分号增加一些属性值或技能熟练度。
...
首先要注意名竞的攻击伤害具有较大的随机性，属性通常和伤害**期望**正相关。具体算法比较复杂，并没有完全破译，只需要明白不需要对攻很高的号打出很低的伤害感到惊讶即可。下面将粗略地解释各个属性的作用（实际上更复杂），表述上指的是“如果这个属性高会有什么用”。这些属性影响战斗过程的详细原理将在进阶教程中介绍。
...
和技能发动频率/概率有关的一个数值被称为“**技能熟练度**”，对每个技能而言是 0-104 的一个整数（0 表示不会这个技能），具体关系在后面的进阶教程中再讲。
