...
首先，各个号在战斗中的真实属性并非其显现得那样。攻、防、敏、魔、抗、智应当减去 36（减完后在 0-63 之间），速 +124。
...
在预行动中，每个号会回复期望值和速正相关的、带有随机性的 SP，以及期望值和智正相关的、带有随机性的 MP。（准确来说 SP 是 0/1/2 中的一个随机数乘上**实际速**，MP 不是很清楚）

当一个号的 SP 达到 2048 时，他会进行行动，比如发动攻击或者发动技能之类的。技能通常需要 MP 达到一定的量（尚不明确）才有可能发动。

## 2.2 伤害计算

各种具有伤害的技能都是有伤害的（我在说什么废话），而它们的伤害计算方式通常为一个随机生成出来的基础伤害数值乘上一个技能倍率，然后再根据对方的防/抗除以一个值。
...
聚气倍率指的是，通常情况下为 1，聚气后为 1.7，实际上还有更复杂的机制，详情见 2.3 节。
...
如果要计算实际伤害，只需要除以 \(64 + 目标防/目标抗\)。防还是抗取决于你的攻击是物理攻击还是魔法攻击。
...
的形式进行表述期望伤害。<br>
实际上这个的意思是，a b c 每一个都是一次根据 2.2 所说的基础伤害计算方式生成的随机数。而正常普通攻击下则是进行这样的计算一次。

## 2.4 命中算法和实际伤害计算

在 2.3 中有提到，对技能命中概率的一般拟合，普通攻击为 \(自身法力/攻击 + 自身敏捷\) / \(目标法抗/防御 + 目标敏捷\)。<br>
实际上这样的表述有误导性。这样看上去像是一个简单的除法，但是，怎么可能呢？<br>
这一节来简要讲一下这个命中算法。

我们规定，普通攻击中，自己的**命中参数**为 \(自身法力/攻击 + 自身敏捷\)，目标的**回避参数**为 \(目标法抗/防御 + 目标敏捷\)。对于特殊技能，命中参数和回避参数按照 2.3 说明的来算。

设命中参数为 `a`，回避参数为 `b`，计算出 `s=max(7,24+b-a)`，如果 `s` 大于 64，那么让 `s=s/4+48`，向下取整。意思是超出 64 的部分每 4 点才能让最终值 + 1。
